    ...
    def update(self, args):
        ...
        if game:
            ...
            if not self.game.is_alive() or self.game.tick >= self.max_tick:
                key = self.game.winner.name
                RESULTS[key] += 1
                with open("./logs/spike11Results.txt", 'a') as file:
...
if __name__ == '__main__':
    ...
    with open("./logs/spike11Results.txt", 'w') as file:
        file.write("Results of wins on 100 maps between Rando and Simple Strategic\n")

    x = 0
    while x < 100:
        gamestate = open('./maps/map{0}.txt'.format(x)).read()
        players = ['Simple_Strategic', 'Rando']
        window = PlanetWarsWindow(gamestate=gamestate, players=players, max_game_length=5000)
        app.run()
        app.exit()
